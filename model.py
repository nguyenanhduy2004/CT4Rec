...
class Model():
    def __init__(self, usernum, itemnum, args, reuse=None):
        ...
        with tf.variable_scope("SASRec", reuse=tf.AUTO_REUSE):
            ...
            # sequence embedding
            seq_input, item_emb_table = embedding(
                self.input_seq,
            ...
            # encoder 2
            seq_encoder_second = self.user_encoder(
                input_seq=seq_input,
                dropout_rate=args.dropout_rate,
                mask=mask,
                num_blocks=args.num_blocks,
        ...
        seq_emb = tf.reshape(
            self.seq,
            [batch_size * args.maxlen, args.hidden_units]
        )

        seq_emb_second = tf.reshape(
            self.seq_second,
            [batch_size * args.maxlen, args.hidden_units]
        )

        seq_emb_l2 = tf.reshape(
            self.seq_l2,
            [batch_size * args.maxlen, args.hidden_units]
        )

        seq_emb_second_l2 = tf.reshape(
            self.seq_second_l2,
            [batch_size * args.maxlen, args.hidden_units]
        )
        ...
        test_item_emb = tf.nn.embedding_lookup(
            item_emb_table,
            self.test_item
        )
        ...
        # ignore padding
        istarget = tf.reshape(
            tf.cast(tf.not_equal(pos, 0), tf.float32),
        ...
        with tf.variable_scope("basic_loss"):
            ...
            self.neg_logits = tf.reshape(
                tf.matmul(neg_emb, tf.expand_dims(seq_emb, -1)),
                [batch_size * args.maxlen, args.neg_sample_n]
            )

            logits_first = tf.concat(
                [self.pos_logits, self.neg_logits],
                axis=-1
            )

            logits_first = tf.clip_by_value(
                logits_first,
                clip_value_min=-80,
                clip_value_max=80
            )
        ...
        with tf.variable_scope("basic_loss2"):
            ...
            self.pos_logits_second = tf.reduce_sum(
                pos_emb * seq_emb_second,
                -1,
                keepdims=True
            )

            self.neg_logits_second = tf.reshape(
                tf.matmul(neg_emb, tf.expand_dims(seq_emb_second, -1)),
                [batch_size * args.maxlen, args.neg_sample_n]
            )

            logits_second = tf.concat(
                [self.pos_logits_second, self.neg_logits_second],
                axis=-1
            )

            logits_second = tf.clip_by_value(
                logits_second,
                clip_value_min=-80,
                clip_value_max=80
            )
            ...
            self.loss_second = self.get_softmax_loss(
                prob=prob_second,
                is_target=istarget
            )
        ...
        with tf.variable_scope("ur_loss"):
            ...
            if args.con_alpha <= 0:
                ...
                cosine_sim = tf.clip_by_value(
                    cosine_sim,
                    0,
                    2.0 - 1e-10
                )

                self.ur_loss = -tf.reduce_mean(
                    tf.log(cosine_sim + 1e-10)
                )
                ...
                user_inner_match_prob2 = tf.clip_by_value(
                ...
                self.ur_loss, _ = self.weight_info_nce(
                    sim=con_sim,
